...
(function (global) {
    ...
    function readLabels(root) {
        ...
        return {
            ...
            lessonOf: d.dzLblLessonOf || 'Lesson {n} of {total}',
            stepOf: d.dzLblStepOf || 'step {s} of {st}',
        };
    }
    ...
    class DobrozalOnboarding {
        ...
        cacheDom() {
            const lessonRoot = this.root.querySelector('[data-dz-view="lesson"]');
            this.el = {
                ...
                lessonContext: this.root.querySelector('[data-dz-lesson-context]'),
                mascot: lessonRoot?.querySelector('[data-dz-mascot]'),
                bubbleTitle: lessonRoot?.querySelector('[data-dz-bubble-title]'),
                bubbleText: lessonRoot?.querySelector('[data-dz-bubble-text]'),
                quizOptions: lessonRoot?.querySelector('[data-dz-quiz-options]'),
                continueBtn: lessonRoot?.querySelector('[data-dz-continue]'),
                backBtn: lessonRoot?.querySelector('[data-dz-back-map]'),
                lessonProgress: lessonRoot?.querySelector('[data-dz-lesson-progress]'),
        ...
        updateLessonContext() {
            if (!this.el.lessonContext || !this.currentLesson) return;
            const total = this.quest.totalLessons || this.allLessons().length;
            const n = this.lessonIndex(this.currentLesson.id);
            const steps = this.currentLesson.steps || [];
            const line =
                this.labels.lessonOf.replace('{n}', String(n)).replace('{total}', String(total)) +
                ' · ' +
                this.currentLesson.title +
                ' · ' +
                this.labels.stepOf
                    .replace('{s}', String(this.currentStepIndex + 1))
                    .replace('{st}', String(steps.length));
            this.el.lessonContext.textContent = line;
        }
