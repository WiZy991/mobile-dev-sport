...
def run_with_session(
    ...
    *,
    ...
    # В режиме «Слушать» без входящего C01 одноразовый клиент шёл бы на c01_host:c01_ws_port
    # (часто .85:80 = веб PERCo), что даёт «HTTP 200» и путает пользователя.
    if eq.connection_mode == "listen":
        raise RuntimeError(
            "В режиме «Слушать» «Прочитать/Записать net» работают только когда C01 уже подключился к агенту "
            "(в шапке «C01: 1/1 подключено»). Пока 0/1 не нажимайте эти кнопки — они не настраивают PERCo по веб-порту.\n\n"
            "Сначала на контроллере в веб-интерфейсе укажите сервер 192.168.0.63:8765 (с портом), "
            "откройте входящий TCP 8765 в брандмауэре Windows, перезагрузите C01 и дождитесь подключения.\n\n"
            "Первичную запись net без связи можно сделать только через режим «К контроллеру» с портом WebSocket из мануала PERCo, "
            "или утилиту производителя — не через :80."
        )
