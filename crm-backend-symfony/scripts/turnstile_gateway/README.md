...
- **Вход по QR**: считыватель → stdin шлюза → `POST /api/v1/gateway/access/entry` → если CRM разрешает
  и в ответе есть `open_device` — шлюз сам шлёт команду в локальный PERCo-Web.
...
| Считыватель пищит, но CRM не получает QR                 | stdin не читается → запустите в терминале или используйте обёртку, которая зовёт `gateway.py qr` |
