...
## Безопасность и проверка Сбером

См. **[SECURITY-AUDIT.md](./SECURITY-AUDIT.md)** — архитектура, меры в коде, что показывать на демо.

## Лог JSON userinfo (отладка, по умолчанию выключен)

Включение: `SBER_USERINFO_LOG=1` в `.env`. Блок **`identification` (паспорт) в лог не пишется** — только `[REDACTED]`.

При включении файл: `/app/var/log/sber-userinfo.log`
